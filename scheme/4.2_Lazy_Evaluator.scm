...
(define (eval-if exp env)
  ...
  (if (eval-true? (actual-value (if-predicate exp) env))
...
(define (evaluated-thunk? p)
  (tagged-list? p 'evaluated-thunk))
...
; Returns non-thunk value, that is not tagged 'thunk or 'evaluated-thunk
(define (actual-value exp env)
  (force-it (eval exp env)))

; returns value if obj is tagged thunk or evaluated-thunk, it modifies.
(define (force-it obj)
  (cond 
   ((thunk? obj)
    (let ((val (actual-value (thunk-exp obj) (thunk-env obj))))
